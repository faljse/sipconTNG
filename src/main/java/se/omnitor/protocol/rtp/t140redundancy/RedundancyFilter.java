...
*/
	...
	while (!done) {
		...
		for (int i=0; i<sosIndex; i++) {
		    output.add(Byte.valueOf(inputString[i]));
		}
		...
		if ( nbrPacketsLost > 0 && (rfcSupport != 1)) {
			...
			for (int i=0;i<(nbrPacketsLost-redLevel);i++) {
			    output.add(Byte.valueOf((byte)0xef));
			    output.add(Byte.valueOf((byte)0xbf));
			    output.add(Byte.valueOf((byte)0xbd));
			}
			...
			for (int i=0;i<redLevel;i++) {
			    ...
			    pos += 2;
			    for (int j=0;j<length;j++) {
				output.add(Byte.valueOf(inputString[rIdx+pos+j]));
			    }
			    pos+=length;
