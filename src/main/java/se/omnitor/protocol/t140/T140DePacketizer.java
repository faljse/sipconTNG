...
public class T140DePacketizer implements Runnable {
    ...
    public void start() {
	if (fifo == null) {
	    logger.severe("A FifoBuffer must be set before T140DePacketizer " +
			  "is started!");
	    return;
	}
	if (eventHandler == null) {
	    logger.severe("An T140EventHandler must be set before " +
			  "T140DePacketizer is started!");
	    return;
	}
	...
	while (running) {
		...
		logger.throwing(this.getClass().getName(), "run", uee);
	    }
