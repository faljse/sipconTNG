...
/**
 * This interface represents the messaging entity of a SIP stack and as such is
 * the interface that defines the messaging and transactional component view of
 * the SIP stack. It must be implemented by any object representing a SIP stack
 * compliant to this specification that interacts directly with a proprietary
 * implementation of a SIP stack. This interface defines the methods that enable
 * any registered application implementing the {@link javax.sip.SipListener}
 * interface to:
 * <ul>
 * <li> Register a {@link javax.sip.SipListener} to the SipProvider. Once the
 * SipListener is registered with the SipProvider it will get notified of Events
 * representing either Request, Response, Timeout messages, Network errors and
 * Transaction termination.
 * <li> De-register a {@link javax.sip.SipListener} from the SipProvider. Once a
 * SipListener is de-registered, it will no longer receive any Events from that
 * SipProvider.
 ...
 * ListeningPoint but cannot have two UDP ListeningPoints. Application that
 * wishes to have multiple ListeningPoints with the same transport must use a
 * separate SipProvider for each such ListeningPoint.
 * <p>
 * The ListeningPoints of the SipProvider specify the local address from where
 * request messages will be sent. When the application sends a request directly
 * from a SipProvider or from an object that the SipProvider created (Dialog or
 * Transaction) the application might not know in advance the transport that
 * this request will use. The transport is often resolved using a DNS server. In
 * the process of sending the request, the remote transport will be resolved.
 * The provider will then be able to send the request from a suitable
 * ListeningPoint according to the resolved transport. If the resolved transport
 * is UDP, the Provider's UDP ListeningPoint will be used. If the resolved
 * address is TCP, the Providers's TCP ListeningPoint will be used.
 * <p>
 * If the application creates a SipProvider with a single ListeningPoint, let's
 * say UDP, it means that UDP requests will be sent from the specific UDP
 * ListeningPoint, and that the application does not care from where TCP
 * requests will be sent. This is left for the SipStack decision.
 * <p>
 * Since the transport might not be known in advance, the application might find
 * it difficult to specify the transport in the Via header. The SipProvider is
 * responsible for fixing the Via header transport if needed. If the application
 * set a sent-by identifier to the ListeningPoint that is different then the
 * sent-by parameter in the message, the sent-by parameter of the message will
 * be updated.
 * <p>
 * If the application created a provider with zero ListeningPoint, it must have
 * only a single SipProvider per SipStack.
 * <p>
 * 
 * A SipProvider has the capability to behave transaction statefully, dialog
 * statefully and statelessly. The transaction stateful methods are defined on
 * the ClientTransaction and ServerTransaction respectfully. The transaction
 * stateful method defined specifically for UAC and stateful proxy applications
 * is:
 * <ul>
 * <li> {@link ClientTransaction#sendRequest()}
 * </ul>
 * <p>
 * The stateful (transactional) convenience method defined specifically for UAS
 * and stateful proxy applications is:
 * <ul>
 * <li> {@link ServerTransaction#sendResponse(Response)}
 * </ul>
 * <p>
 * The dialog stateful methods defined specifically for UAC and stateful proxy
 * applications are:
 * <ul>
 ...
 * Dialog-Creating Request i.e. INVITE. Instead the server transaction is set to
 * <code>null</code> in the RequestEvent and the RequestEvent also containing
 ...
 * Non-Dialog-Creating Request upon which this application has already responded
 * to the Dialog-Creating Request of the same dialogue the server transaction is
 * automatically placed to the RequestEvent and passed up to the application
 * upon which it can respond. Note that the server transaction may be null in a
 * stateful implementation if the incoming request does not match any dialog but
 * must be part of one. That is for requests that must have state but for which
 * the stack cannot find that state, the application can still handle these
 * requests statelessly. The application cannot create a new server transaction
 * for such requests.
 * </ul>
 * <p>
 ...
 * 18.1.1 of <a href = "http://www.ietf.org/rfc/rfc3261.txt">RFC3261</a>.
 * <p>
 ...
 * client transport uses the matching procedures of Chapter 17 of <a href =
 * "http://www.ietf.org/rfc/rfc3261.txt">RFC3261</a> to attempt to match the
...
public interface SipProvider {
	 ...
	 * @throws SipException
	 *             if the SipProvider cannot send the Response for any reason.
	 * @see Response
	 * @since v1.1
	 */
	...
	/**
	 * Create a dialog for the given transaction. This method is only called
	 * when AUTOMATIC_DIALOG_SUPPORT is off. This method is invoked when the
	 * application wants to explicitly manage the association between
	 * transaction and dialog. This must may only be called on a dialog-creating
	 * transaction. Dialogs are created in advance, before any responses are
	 * sent or received, using the initial client or server transaction. The
	 * Dialog state is set to null when the dialog is created. The server side
	 * of a dialog calls this method before sending out the response to a dialog
	 * creating request. The client side of the dialog calls this method before
	 * sending out the initial request via the dialog creating transaction. The
	 * caller is required to set up the tags and other information in the
	 * request/response before calling this method.
	 * 
	 * <p>
	 * For UAC's Forked calls are handled as follows: The response of a forked
	 * call that completes the initially created dialog will use the original
	 * dialog that is associated with the transaction. Subsequent responses that
	 * correspond to other branches of the fork ( ie. with the same From header
	 * tag, and Call ID but different To header tags) result in the creation of
	 * additional dialogs that are associated with these responses. The created
	 ...
	 * 
	 * @throws SipException
	 *             if one or more of the following is true: <br/>
	 *             <ol>
	 *             <li>The Method of the Request is not a Dialog creating </li>
	 *             <li>There is missing required information such as From
	 *             header Tag in the Request </li>
	 *             <li>This method is called after the response recieved on the
	 *             client side </li>
	 *             <li>This method is called after the response is sent out on
	 *             the server side of the dialog. </li>
	 *             </ol>
	 * 
	 * @since v1.2
	 */
