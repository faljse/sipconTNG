...
public class StringMsgParser {
	 ...
	 * parseSIPMessage(String) method is invoked then the body is assumed to be
	 * a string.
	 */
	...
	/**
	 * This is called repeatedly by parseSIPMessage to parse the contents of a
	...
	private SIPMessage parseMessage(String currentMessage)
		...
		if (!firstLine.startsWith(SIPConstants.SIP_VERSION_STRING)) {
			...
			try {
				...
				if (this.parseExceptionListener != null)
					this.parseExceptionListener.handleException(ex, sipmsg,
							RequestLine.class, firstLine, currentMessage);
				else
					throw ex;
			...
			try {
				...
				if (this.parseExceptionListener != null) {
					this.parseExceptionListener.handleException(ex, sipmsg,
							StatusLine.class, firstLine, currentMessage);
				} else
					throw ex;
		...
		for (int i = 1; i < messageHeaders.size(); i++) {
			...
			try {
				...
				if (this.parseExceptionListener != null) {
					...
					if (hdrClass == null) {
						hdrClass = ExtensionHeaderImpl.class;

					}
					this.parseExceptionListener.handleException(ex, sipmsg,
							hdrClass, hdrstring, rawMessage);
